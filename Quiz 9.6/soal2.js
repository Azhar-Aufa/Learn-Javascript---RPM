...
function shoppingTime(memberId, money) {
    ...
    if(memberId === '' || memberId === undefined) return "Mohon maaf, toko X hanya berlaku untuk member saja";
    else if (money < barang[4].harga ) return "Mohon maaf, uang tidak cukup";
    else {
        let result = {
            memberId: memberId,
            money: money,
        ...
        for(let i = 0; i <= barang.length -1; i++) {
            if(result.changeMoney >= barang[i].harga) {
            result.changeMoney -= barang[i].harga;
            result.listPurchased.push(barang[i].namaBarang)
            }
        }
        return result
    }
