...
function countProfit(shoppers) {
    ...
    let buyer = shoppers;
    if(buyer.length === 0) return shoppers;
    else {
        let results = [{
        ...
        for(let i = 0; i <= buyer.length -1; i++) {
            for(let j = 0; j <= results.length -1; j++) {
                if(buyer[i].product === results[j].product && buyer[i].amount <= results[j].leftOver) {
                    results[j].shoppers.push(buyer[i].name);
                    results[j].leftOver -= buyer[i].amount;
                    results[j].totalProfit += (listBarang[j][1] * buyer[i].amount)
                }
            }
        }
        return results;
